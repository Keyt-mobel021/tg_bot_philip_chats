...
def chat_detail_keyboard(chat_id: int, is_frozen: bool, is_admin_or_manager: bool):
    ...
    if is_admin_or_manager:
        ...
        # ЗАДАЧА 5: кнопка "📝 Описание" только для админа
...
def freeze_confirm_keyboard(chat_id: int, is_frozen: bool):
    builder = InlineKeyboardBuilder()
    action = ChatAction.freeze_confirm
    builder.button(text="✅ Да", callback_data=ChatCD(action=action, chat_id=chat_id, page=1))
    builder.button(text="❌ Нет", callback_data=ChatCD(action=action, chat_id=chat_id, page=0))
    builder.adjust(2)
    return builder.as_markup()
...
# ──────────────────────────────────────────────
#  ЗАДАЧА 1: Кнопки под рассылкой (написать/история)
# ──────────────────────────────────────────────
...
def member_detail_keyboard(chat_id: int, member_id: int, is_blocked: bool):
    builder = InlineKeyboardBuilder()
...
def add_member_keyboard(chat_id: int, profiles: list, users: list):
    """Список профилей + юзеров для добавления в чат"""
    builder = InlineKeyboardBuilder()
    # ЗАДАЧА 4: кнопка «Пригласить по ссылке» — теперь создаёт многоразовую ссылку
...
def profile_type_keyboard():
    """Выбор типа профиля при создании"""
